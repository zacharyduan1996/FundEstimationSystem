...
import { z } from "zod";
import { deleteFundReviewNote, hasWatchlistItem, ReviewNoteNotFoundError, updateFundReviewNote } from "@/lib/db";
...
export async function PUT(
  ...
  try {
    const note = updateFundReviewNote({
      id,
      code,
    ...
    if (error instanceof ReviewNoteNotFoundError) {
      return NextResponse.json({ error: "复盘笔记不存在" }, { status: 404 });
    }
    const message = error instanceof Error ? error.message : "更新复盘失败";
...
export async function DELETE(
  ...
  const deleted = deleteFundReviewNote(id, code);
  if (!deleted) {
    return NextResponse.json({ error: "复盘笔记不存在" }, { status: 404 });
  }
